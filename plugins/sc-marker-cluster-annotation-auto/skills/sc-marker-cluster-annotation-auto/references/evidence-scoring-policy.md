...
- If coherent expected-parent and off-parent programs coexist, return their nearest shared ancestor, set `formal_identity_fallback=mixed_parent_off_parent_lineages`, `mixed_population=true`, and `auto_merge_allowed=false`; use cell-level coexpression or reclustering to distinguish a mixed cluster from doublets.
...
- An APC-high cluster with both `CD1C/CLEC10A/FCER1A` and `CD14/FCN1/VCAN/FCGR3A/LST1` is a DC3 boundary candidate. Cluster-level ratios and literature do not establish same-cell coexpression; require cell-level validation or resolving reclustering and block formal delivery/automatic merging until resolved.
...
- Permit a configured Immature_B absolute program at recombination exit: require a coherent surface-IgM/maturation bridge and retained B identity, then block it only when multiple early-recombination anchors remain both detectable and dataset-dominant. This prevents weak residual recombination from forcing a spatially and biologically later cluster back to Pre_B.
...
- If coherent CD4 and CD8 alpha-beta programs coexist, return their common `T_cell` ancestor, set mixed/suspected-doublet review, and block automatic merging until cell-level coexpression or reclustering resolves the components.
...
Treat mutually exclusive T-cell receptor branches as a mixed-population risk even though they share the same broad T lineage. In full detection-ratio mode, coherent CD4/CD8 alpha-beta identity together with at least two gamma-delta TCR anchors (`TRDC/TRGC1/TRGC2`) and a near-scoring `gdT` candidate requires `R2_RECLUSTER_OR_DOUBLET_REVIEW`, `mixed_population=true`, and `auto_merge_allowed=false`. Fall the formal identity back to the nearest shared parent such as `T_cell`; retain both leaf candidates in `possible_components`. Cluster-level proportions flag this risk but do not establish whether the programs occur in separate cells or doublets.
