...
Background expression is a dataset-wide problem, not a lineage-specific exception. A secondary identity program may support a mixed/off-parent call only when it clears the common competing-program gate: coherent review-level anchors, multiple dataset-relative strong anchors, positive specificity above the configured floor, sufficient score relative to the primary identity, and a distinct ontology branch. Broad ancestors such as `NK_cell` support a coherent descendant such as `CD56dim_NK`; they are not independent rivals or mixture components. Apply this rule equally to TCR, cytotoxic, myeloid, epithelial, endothelial, stromal, and other programs.
