...
The final workbook contains `注释结果`, `详细证据`, `说明与数据来源`, and `细胞类型与文献`. The research sheet lists involved cell types, characteristic markers, candidates/competitors, reusable literature sources, and UMAP judgment. The main sheet keeps identity, developmental stage, state, quality score, mixed/doublet risk, and concise judgment evidence, without JSON, full paths, blank manual-entry columns, or duplicated mapping sheets. Automatic filters are disabled and row heights are compact. Every impurity, `Multi_cell`, or abnormal non-pure row must apply a static red fill to `中文名称`; this is a visual warning only and must not modify the annotation text. QA records cluster order, label normalization, knowledge-base/core/config versions and hashes, cross-species calls, mixed/doublet blocks, and identity-state separation.
