...
- `publish --confirm-publish --confirm-merge`: use after that single combined authorization. After pushing, wait until every observed GitHub check/status succeeds, require the PR to be open, non-draft, and cleanly mergeable, merge with the exact head SHA, fetch and verify stable `main`, safely fast-forward a clean registered stable source, then refresh the complete local callable cache from a temporary detached stable worktree. Never overwrite a dirty or divergent registered source, and restore the original marketplace registration even when installation fails. Use `--confirm-publish --create-pr` instead only when the user explicitly requests the narrower PR-only outcome.
